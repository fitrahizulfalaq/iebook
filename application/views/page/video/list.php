<div id="appCapsule">
    <div class="section full gradientSection">
        <div class="in">
            <h1 class="total">الفيديوهات</h1>
        </div>
    </div>

    <div class="section mt-4">
        <?php $this->view('message') ?>
        <div class="textArabic">
            <p>مجموعة فيديوهات الخطابة العربية</p>
            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; تتنـوع فنـون الخطابـة العربية وفقـاً&nbsp;لمـدى الاحتيـاج لهـا. فتتنـوع موضوعات الخطابـة العربية التي يلقيها الخطيب. فهذه هي بعض الأمثلة عن فيديوهات الخطابة العربية.</p>
        </div>
        <div class="goals">
            <!-- item -->
            <div class="item bg-primary">
                <div class="in">
                    <h1>
                        <a href="<?= base_url("page/video/1") ?>" class="text-white">
                            <ion-icon name="arrow-back-circle-outline"></ion-icon>
                        </a>
                    </h1>
                    <div>
                        <a href="<?= base_url("page/video/1") ?>">
                            <h2 class="text-white">&zwnj;أ.&nbsp; الفيديوهات عن المجاملة قبل السلام</h2>
                        </a>
                    </div>
                </div>
            </div>
            <!-- * item -->
            <!-- item -->
            <div class="item bg-primary">
                <div class="in">
                    <h1>
                        <a href="<?= base_url("page/video/2") ?>" class="text-white">
                            <ion-icon name="arrow-back-circle-outline"></ion-icon>
                        </a>
                    </h1>
                    <div>
                        <a href="<?= base_url("page/video/2") ?>">
                            <h2 class="text-white">&zwnj;ب.&nbsp; الفيديوهات عن المقدمة في الخطابة</h2>
                        </a>
                    </div>
                </div>
            </div>
            <!-- * item -->
            <!-- item -->
            <div class="item bg-primary">
                <div class="in">
                    <h1>
                        <a href="<?= base_url("page/video/3") ?>" class="text-white">
                            <ion-icon name="arrow-back-circle-outline"></ion-icon>
                        </a>
                    </h1>
                    <div>
                        <a href="<?= base_url("page/video/3") ?>">
                            <h2 class="text-white">&zwnj;ج.&nbsp;&nbsp; الفيديوهات عن التعبيرات بعد المقدمة</h2>
                        </a>
                    </div>
                </div>
            </div>
            <!-- * item -->
            <!-- item -->
            <div class="item bg-primary">
                <div class="in">
                    <h1>
                        <a href="<?= base_url("page/video/4") ?>" class="text-white">
                            <ion-icon name="arrow-back-circle-outline"></ion-icon>
                        </a>
                    </h1>
                    <div>
                        <a href="<?= base_url("page/video/4") ?>">
                            <h2 class="text-white">&zwnj;د.&nbsp; الفيديوهات عن الاختتام في الخطابة</h2>
                        </a>
                    </div>
                </div>
            </div>
            <!-- * item -->
        </div>
    </div>
</div>
<br>
<br>